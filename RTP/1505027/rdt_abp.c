...
{
    printf("\n#######\nB input here is the packet I received:\nSequence number: %d\nChecksum: %d\nMessage: %s\nThe ACK B is expecting to send is %d\n#######\n",packet.seqnum,packet.checksum,packet.payload,recv_seq);

    if(packet.seqnum == recv_seq && packet.checksum != generate_checksum(packet)){
        struct pkt packet;
        packet.acknum = -1;
        tolayer3(B, packet);
		return;
    }
    else if(packet.seqnum == recv_seq && packet.checksum == generate_checksum(packet)){
        struct msg message;
        recv_seq = 1 - recv_seq;
