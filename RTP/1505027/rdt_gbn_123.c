...
void print_packet(int AorB, char *action,struct pkt packet)
{
...
/* construct a packet */
struct pkt construct_packet(int AorB,struct msg message)
{
	struct pkt packet;	
...
void interrupt_multitimer(int AorB)
{
	timers_running[AorB] = 0;
}

/* start a timer for a packet */
void start_multitimer(int AorB, int seqnum)
{
	/* bound check */
...
void stop_multitimer(int AorB, int seqnum)
{
	/* bound check */
...
void output(int AorB,struct msg message)
{
	...
	if (packet_in_buffer[AorB] < MAX_WINDOW) {
        ...
        printf("\n#######\n[%d] is going to send the following packet:\nSequence number: %d\nChecksum: %d\nMessage: %s\nThe ACK A is expecting to see is %d\n#######\n",AorB,packet.seqnum,packet.checksum,packet.payload,expect_ack[AorB]);
...
/*			if (DEBUG)
        ...
        printf("\n##########\n[%d] has received the ACK packet!\nACK: %d\nCurrent expected ACK: %d\n########\n",AorB,packet.acknum,expect_ack[AorB]);
