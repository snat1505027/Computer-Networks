...
void A_input(struct pkt packet)
{
    if(sender.recv_seq == packet.seqnum && packet.checksum == generate_checksum(packet)){
        ...
        int flag = 0;
        if ((sender.ack_seq <= packet.acknum 
        && packet.acknum < sender.send_seq) 
        || (sender.send_seq<sender.ack_seq && sender.ack_seq <= packet.acknum) || (packet.acknum < sender.send_seq 
        && sender.send_seq < sender.ack_seq))
                flag =1;
        else{
                flag = 0;
        }
        ...
        while(sender.pck_num <WINDOW_SIZE && flag){
...
{
    sender.now_timer = 0;
    for(int i = sender.ack_seq; i<sender.send_seq;i++){
        if( i!= sender.ack_seq){
...
void B_input(struct pkt packet)
{
    ...
    if(receiver.recv_seq == packet.seqnum && packet.checksum == generate_checksum(packet)){
        ...
        int flag = 0;
        if ((receiver.ack_seq <= packet.acknum 
        && packet.acknum < receiver.send_seq) 
        || (receiver.send_seq<receiver.ack_seq && receiver.ack_seq <= packet.acknum) || (packet.acknum < receiver.send_seq 
        && receiver.send_seq < receiver.ack_seq))
                flag =1;
        else{
                flag = 0;
        }
        ...
        while(receiver.pck_num <WINDOW_SIZE && flag){
            struct msg message = removeQueue(A);
...
void B_timerinterrupt(void)
{
    ...
    for(int i = receiver.ack_seq; i<receiver.send_seq;i++){
